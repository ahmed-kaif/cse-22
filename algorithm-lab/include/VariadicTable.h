 ...
 * vt.print();
 */
template <class... Ts> class VariadicTable {
public:
  ...
  VariadicTable(std::vector<std::string> headers,
      ...
      : _headers(headers), _num_columns(std::tuple_size<DataTuple>::value),
        _static_column_size(static_column_size), _cell_padding(cell_padding) {
   ...
   */
  void
  setColumnFormat(const std::vector<VariadicTableColumnFormat> &column_format) {
...
protected:
  ...
  void size_each(TupleType &&, std::vector<size_t> & /*sizes*/,
                 std::integral_constant<
                     size_t, std::tuple_size<typename std::remove_reference<
                                 TupleType>::type>::value>) {}
